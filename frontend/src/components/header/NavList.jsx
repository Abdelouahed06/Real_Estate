...
const NavList = ({SetShowSideBar}) => {
    const isLogin = JSON.parse(localStorage.getItem('token'));
    const dispatch = useDispatch()
    const handleLogout = async () => {
        await dispatch(fetchLogout(isLogin))
    };
    return (
        <ul  className="md:absolute top-14 z-20 left-[-12px] bg-white md:border border-solid  border-gray-200 md:shadow-md rounded-md md:py-2 py-4">
            <Link to='/my-announces'>
                <li onClick={() => SetShowSideBar(false)} className="flex items-center justify-between whitespace-nowrap px-4 md:py-2 py-4 hover:bg-gray-200 cursor-pointer">
                    <div className="flex items-center gap-2">
                    <svg class="av-icon" height="19" width="19" style={{ fill: 'rgb(74, 74, 74)', stroke: 'rgb(74, 74, 74)', strokeWidth: '0px' }} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path 
                    ...
                    <svg className="md:hidden block" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 48 48">
                        <path fill="none" stroke="#000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M19 12L31 24L19 36"></path>
                    </svg>
                </li>
            </Link>

            <Link to='/my-favourites'>
                <li onClick={() => SetShowSideBar(false)} className="flex items-center justify-between px-4 md:py-2 py-4 hover:bg-gray-200 cursor-pointer">
                    <div className="flex items-center gap-2">
                        <svg height="19" width="19" style= {{ fill: 'rgb(74, 74, 74)', stroke: 'rgb(74, 74, 74)', strokeWidth: '0px' }} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" >
                            <path 
            ...
            <li className="flex items-center justify-between px-4 md:py-2 py-4 hover:bg-gray-200 cursor-pointer" onClick={handleLogout}>
                <div className="flex items-center gap-2">
                <svg height="19" width="19" style={{ fill: 'rgb(74, 74, 74)', stroke: 'rgb(74, 74, 74)', strokeWidth: '0px' }} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" >
                    <path 
                        fill="none" d="M0 0h24v24H0z"></path><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2a9.985 9.985 0 018 4h-2.71a8 8 0 10.001 12h2.71A9.985 9.985 0 0112 22zm7-6v-3h-8v-2h8V8l5 4-5 4z">
                    </path>
                </svg>
                    <span>Log Out</span>  
                </div>
